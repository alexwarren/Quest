...
namespace WebEditor2.Controllers
{
    public class EditController(IConfiguration configuration, ILogger logger) : Controller
    {
        ...
        public ActionResult Game(int? id)
        {
            ...
            model.ErrorRedirect = configuration["WebsiteHome"] ?? "http://textadventures.co.uk/";
        ...
        public JsonResult Load(int id, bool simpleMode)
        {
            ...
            if (!result.Success)
            {
                logger.LogInformation("Failed to load game {0} - {1}", id, result.Error);
        ...
        {
            logger.LogDebug("{0}: Get scripts JSON", id);
            return Json(EditorDictionary[id].GetScriptAdderJson());
        }
        ...
        {
            logger.LogDebug("{0}: SaveElement {1}", element.GameId, element.Key);
            if (!element.Success)
            {
                logger.LogError("Element save failed");
                return Timeout();
            }
        ...
        {
            return Json(EditorDictionary[id].GetElementTreeForJson());
        }
        ...
        {
            return View(new WebEditor2.Models.FileUpload
            {
                GameId = id,
        ...
        public ActionResult FileUpload(FileUpload fileModel)
        {
            ...
            {
                logger.LogError("FileUpload - game id {0} not in EditorDictionary", fileModel.GameId);
